...
void* _playbackThread(void* arg) {
    ...
    while (soundData->isPlaying && !soundData->shouldStop) {
        ...
        if (soundData->pcm_handle) {
            ...
            if (soundData->currentPosition + bytes_to_write > soundData->bufferSize) {
                bytes_to_write = soundData->bufferSize - soundData->currentPosition;
                frames_to_write = bytes_to_write / 4;
            }
            
            if (bytes_to_write > 0) {
                // Apply volume
                short* samples = (short*)(soundData->audioBuffer + soundData->currentPosition);
                ...
                snd_pcm_sframes_t frames_written = snd_pcm_writei(soundData->pcm_handle, samples, frames_to_write);
            ...
            if (soundData->currentPosition >= soundData->bufferSize) {
                if (soundData->loopCount == -1 || soundData->currentLoop < soundData->loopCount - 1) {
                    soundData->currentPosition = sizeof(WaveHeader); // Skip header
...
int _Platform_playSound(const char* soundFile, int loopCount, void** platformData) {
    ...
    if (fread(soundData->audioBuffer, 1, soundData->bufferSize, soundData->audioFile) != soundData->bufferSize) {
    ...
    if (err < 0) {
        fprintf(stderr, "Failed to allocate hw params: %s\n", snd_strerror(err));
    ...
    snd_pcm_hw_params_set_access(soundData->pcm_handle, soundData->hw_params, SND_PCM_ACCESS_RW_INTERLEAVED);
    snd_pcm_hw_params_set_format(soundData->pcm_handle, soundData->hw_params, SND_PCM_FORMAT_S16_LE);
    snd_pcm_hw_params_set_channels(soundData->pcm_handle, soundData->hw_params, 2);
    unsigned int rate = 44100;
    snd_pcm_hw_params_set_rate_near(soundData->pcm_handle, soundData->hw_params, &rate, 0);
    ...
    if (err < 0) {
        fprintf(stderr, "Failed to prepare PCM device: %s\n", snd_strerror(err));
        snd_pcm_close(soundData->pcm_handle);
        free(soundData->audioBuffer);
        pthread_mutex_destroy(&soundData->mutex);
        pthread_cond_destroy(&soundData->cond);
