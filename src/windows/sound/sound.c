...
void CALLBACK _waveOutCallback(HWAVEOUT hwo, UINT uMsg, DWORD_PTR dwInstance, DWORD_PTR dwParam1, DWORD_PTR dwParam2) {
    ...
    if (uMsg == WOM_DONE && soundData) {
        ...
        if (soundData->isPlaying && !soundData->isPaused) {
            ...
            if (soundData->loopCount == -1 || soundData->currentLoop < soundData->loopCount - 1) {
                ...
                waveOutUnprepareHeader(soundData->hWaveOut, &soundData->waveHeader, sizeof(WAVEHDR));
                ...
                waveOutPrepareHeader(soundData->hWaveOut, &soundData->waveHeader, sizeof(WAVEHDR));
                waveOutWrite(soundData->hWaveOut, &soundData->waveHeader, sizeof(WAVEHDR));
            } else {
...
int _Platform_playSound(const char* soundFile, int loopCount, void** platformData) {
    ...
    // Open file
    soundData->hFile = CreateFileW(wideFileName,
                                  GENERIC_READ,
                                  FILE_SHARE_READ,
                                  NULL,
                                  OPEN_EXISTING,
    ...
    if (soundData->hFile == INVALID_HANDLE_VALUE) {
        fprintf(stderr, "Failed to open audio file '%s': %lu\n", soundFile, GetLastError());
        DeleteCriticalSection(&soundData->criticalSection);
        free(soundData);
        return -1;
    }
    ...
    // Create file mapping
    soundData->hMapFile = CreateFileMapping(soundData->hFile,
                                           NULL,
                                           PAGE_READONLY,
                                           0,
                                           0,
    ...
    // Map file to memory
    soundData->lpMapAddress = MapViewOfFile(soundData->hMapFile,
                                           FILE_MAP_READ,
                                           0,
                                           0,
                                           0);
    ...
    soundData->waveFormat.nBlockAlign = soundData->waveFormat.nChannels * (soundData->waveFormat.wBitsPerSample / 8);
    soundData->waveFormat.nAvgBytesPerSec = soundData->waveFormat.nSamplesPerSec * soundData->waveFormat.nBlockAlign;
    soundData->waveFormat.cbSize = 0;
    
    // Open wave output device
    MMRESULT result = waveOutOpen(&soundData->hWaveOut,
                                 WAVE_MAPPER,
    ...
    // Prepare header
    result = waveOutPrepareHeader(soundData->hWaveOut, &soundData->waveHeader, sizeof(WAVEHDR));
    if (result != MMSYSERR_NOERROR) {
    ...
    // Start playback
    result = waveOutWrite(soundData->hWaveOut, &soundData->waveHeader, sizeof(WAVEHDR));
    if (result != MMSYSERR_NOERROR) {
