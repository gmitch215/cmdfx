 ...
 * @date 2025-03-22
 *
 * This is a simple game where you control a square with WASD and change its
 * color with the number keys.
 *
...
int onKeyPress(CmdFX_Event* event) {
    ...
    switch (c) {
        case '0': Sprite_setForegroundAll(square, 0xFFFFFF); break;
        case '1': Sprite_setForegroundAll(square, 0xFF0000); break;
        case '2': Sprite_setForegroundAll(square, 0x00FF00); break;
        case '3': Sprite_setForegroundAll(square, 0x0000FF); break;
        case '4': Sprite_setForegroundAll(square, 0xDB09F7); break;
        case '5': Sprite_setForegroundAll(square, 0x09EBF7); break;
        case '6': Sprite_setForegroundAll(square, 0xFFD700); break;
        case '7': Sprite_setForegroundAll(square, 0xFFA700); break;
        case '8': Sprite_setForegroundAll(square, 0x9C09F7); break;
        case '9': Sprite_setForegroundAll(square, 0x222222); break;
    }
...
int main() {
    ...
    square = Sprite_create(
        ...
        String2DBuilder_create(
            3, 3
        ), // Initialize ANSI codes to be empty (specify no color)
        0
    );
    Sprite_draw(
        width / 2, height / 2,
    ...
    printf(
        "Use WASD to move the square and the number keys to change its "
        "color.\nPress 'Q' to "
        "quit.\n"
    );

    while (running) {
    } // Hang the application until 'Q' is pressed

    shutdownCmdFXEvents(); // Once the application is done, free the event
                           // listeners
    return 0;
}