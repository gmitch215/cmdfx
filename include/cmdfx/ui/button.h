 ...
 */
typedef void (*CmdFX_ButtonCallback)(
    void* button, CmdFX_MouseEvent* event, unsigned long long time
);
 ...
 * while also including a callback function to be called when the button is
 * clicked.
 */
typedef struct CmdFX_Button {
     ...
     * are static sprites, meaning they are not affected by the physics
     * engine.
     */
    CmdFX_Sprite* sprite;
    /**
     ...
     *
     * This is the same as `sprite->x`, but is used to determine the
     * position of the button on the screen. This value is set when the
     * button is drawn.
     */
    int x;
    /**
     ...
     *
     * This is the same as `sprite->y`, but is used to determine the
     * position of the button on the screen. This value is set when the
     * button is drawn.
     */
    int y;
     ...
     *
     * If the button is removed or not on the screen, this value will be
     * `-1`.
     */
    int id;
    /**
     ...
     *
     * This is used to identify the type of button. This value is set when
     * the button is created and can be used to store any additional data
     * that is needed for the button. It should not be changed by the user.
     */
    int type;
    /**
     ...
     * to help determine what to do with the button. This value is set when
     * the button is created and can be used to store any additional data
     * that is needed for the button. It should not be changed by the user.
     */
    void* extra;
 ...
 *
 * @return An array of pointers to the registered buttons on the terminal
 * screen.
 */
 ...
 *
 * This function creates a new button with the given sprite and callback
 * function. The button is not added to the UI manager, so it must be added
 * manually.
 *
 ...
 * character, and callback function. The button is not added to the UI
 * manager, so it must be added manually.
 *
 ...
 * will no longer be rendered on the screen and will not respond to mouse
 * events.
 *
 ...
 *
 * The array must freed after use. The array is terminated with a NULL
 * pointer.
 *
 * @param x The x position to check.
 * @param y The y position to check.
 * @return An array of pointers to the buttons at the given position, or
 * NULL if no buttons are found.
 */
 ...
 * @param x The x position to check.
 * @param y The y position to check.
 * @return A pointer to the button at the given position, or NULL if no
 * button is found.
 */
