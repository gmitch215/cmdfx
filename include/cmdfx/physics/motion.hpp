...
namespace CmdFX {
    ...
    /**
     * @brief A C++ wrapper around a CmdFX_Sprite struct for motion-related functions.
     * 
     * This class provides methods to get and set the velocity and acceleration of a sprite,
     * check for potential collisions, and reset the motion of a sprite. The destructor will
     * reset all motion when the object is destroyed.
     */
    class SpriteMotion final {
        ...
        public:
            ...
            SpriteMotion(CmdFX_Sprite* sprite) : sprite(std::make_unique<Sprite>(sprite)) {}
            ~SpriteMotion() {
                if (sprite) { Sprite_resetAllMotion(sprite->getSprite()); }
            }
             ...
             * @return A vector containing the motion values, or an empty vector if the sprite has no motion array or an error occurred.
             */
            ...
            std::vector<std::unique_ptr<Sprite>> getAboutToCollideSprites() {
                ...
                CmdFX_Sprite** arr = Sprite_getAboutToCollideSprites(sprite->getSprite());
                if (arr) {
